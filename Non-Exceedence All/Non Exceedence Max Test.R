...
quantilemid<-data.frame(matrix(ncol=43,nrow=5))
quantilehist<-data.frame(matrix(ncol=43,nrow=5))
quantilediff<-data.frame(matrix(ncol=43,nrow=5))
quantilepdiff<-data.frame(matrix(ncol=43,nrow=5))
for (k in 2:43)
{
...
for (l in 2:43)
{
  for (m in 1:5)
  {
    quantilediff[m,l-1]<-quantilemid[m,l]-quantilehist[m,l]
    quantilepdiff[m,l-1]<-(quantilemid[m,l]-quantilehist[m,l])/quantilehist[m,l]
  }
}

percentshist<-melt(quantilehist)
percentshist[,1]<-factor(rep(c("5%","25%","50%","75%","95%"),42),levels=c("5%","25%","50%","75%","95%"))
percentshist[,2]<-factor(c(rep("stn1",70),rep("stn34",70),rep("stn77",70)))

percentsmid<-melt(quantilemid)
percentsmid[,1]<-factor(rep(c("5%","25%","50%","75%","95%"),42),levels=c("5%","25%","50%","75%","95%"))
percentsmid[,2]<-factor(c(rep("stn1",70),rep("stn34",70),rep("stn77",70)))

percentsdiff<-melt(quantilediff)
percentsdiff[,1]<-factor(rep(c("5%","25%","50%","75%","95%"),42),levels=c("5%","25%","50%","75%","95%"))
percentsdiff[,2]<-factor(c(rep("stn1",70),rep("stn34",70),rep("stn77",70)))

percentspdiff<-melt(quantilepdiff)
percentspdiff[,1]<-factor(rep(c("5%","25%","50%","75%","95%"),42),levels=c("5%","25%","50%","75%","95%"))
percentspdiff[,2]<-factor(c(rep("stn1",70),rep("stn34",70),rep("stn77",70)))

names(percentshist)<-c("Percent","Station","Value")
names(percentsmid)<-c("Percent","Station","Value")
names(percentsdiff)<-c("Percent","Station","Value")
names(percentspdiff)<-c("Percent","Station","Value")

hist<-ggplot(aes(y = Value, x = Percent, fill=Station),data=percentshist)+geom_boxplot()+
  ylab(label="Precipitation (mm/day)")+xlab(label="Quantiles")+theme_bw()+
  scale_y_continuous(breaks = seq(0,200,by=50),limits=c(0,200),expand=c(0,0))+
  ggtitle(label="Historical Data")

mid<-ggplot(aes(y = Value, x = Percent, fill=Station),data=percentsmid)+geom_boxplot()+
  ylab(label="Precipitation (mm/day)")+xlab(label="Quantiles")+theme_bw()+
  scale_y_continuous(breaks = seq(0,250,by=50),limits=c(0,250),expand=c(0,0))+
  ggtitle(label="Midcentury Data")
  
diff<-ggplot(aes(y = Value, x = Percent, fill=Station),data=percentsdiff)+geom_boxplot()+
  ylab(label="Precipitation (mm/day)")+xlab(label="Quantiles")+theme_bw()+
  scale_y_continuous(breaks = seq(-25,100,by=25),limits=c(-25,100),expand=c(0,0))+
  ggtitle(label="Difference in Mid/Hist")
  
pdiff<-ggplot(aes(y = Value, x = Percent, fill=Station),data=percentspdiff)+stat_boxplot(geom="errorbar")+
  geom_boxplot()+ylab(label="Precipitation (mm/day)")+xlab(label="Quantiles")+theme_bw()+
  scale_y_continuous(breaks = seq(-0.5,1.0,by=0.25),limits=c(-0.5,1.0),expand=c(0,0))+
  ggtitle(label="Percent Diff in Mid/Hist")