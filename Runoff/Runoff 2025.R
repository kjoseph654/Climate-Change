...
for(gcm in gcms)
{
  data.in<-paste("D:/RStudio/Climate Change/Climate Change/BCSDm Runoff W 2025/",gcm,"_rcp85_totalrunoff_stn_W_2025-2054_HD.csv",sep="")
...
for(gcm in gcms)
{
  data.in<-paste("D:/RStudio/Climate Change/Climate Change/BCSDm Runoff E 2025/",gcm,"_rcp85_totalrunoff_stn_E_2025-2054_HD.csv",sep="")
...
gcms1<-c("canesm2","cnrm-cm5","csiro-mk3-6-0","gfdl-esm2g","inmcm4","miroc5","mpi-esm-lr","mri-cgcm3","noresm1-m")
name2<-c("date")
runoff3<-data.frame(NA)

for(gcm1 in gcms1)
{
  data.in<-paste("D:/RStudio/Climate Change/Climate Change/LOCA Runoff W 2025/",gcm1,"_rcp85_totalrunoff_stn_W_2025-2054_HD.csv",sep="")
...
for(gcm1 in gcms1)
{
  data.in<-paste("D:/RStudio/Climate Change/Climate Change/LOCA Runoff E 2025/",gcm1,"_rcp85_totalrunoff_stn_E_2025-2054_HD.csv",sep="")
...
quant.runoff1<-data.frame(matrix(nrow=41,ncol=9))
for (i in 2:10)
{
...
data.in<-paste("D:/RStudio/Climate Change/Climate Change/SCVWDMonthlyReservoirInflowVolumes_EW.csv",sep="")
...
names(data1.runoff)<-c("Quantile","Median BCSDm","Median LOCA","Observed")
...
ggplot(aes(y = value, x = Quantile, colour=variable),data=data1.runoff)+theme_bw()+geom_line()+
  ...
  geom_ribbon(data=data2.runoff,aes(x=Quantile,ymin=variable,ymax=value, fill="Range BCSDm"),alpha=0.2,colour="deepskyblue")+
  geom_ribbon(data=data4.runoff,aes(x=Quantile,ymin=variable,ymax=value, fill="Range LOCA"),alpha=0.2,colour="grey60")+
  scale_fill_manual(name="",values=c("Range BCSDm"="deepskyblue", "Range LOCA"="grey60"))+
  ...
  ylab("Inflow (1000 ac-ft)")+xlab("Quantiles")+coord_flip()